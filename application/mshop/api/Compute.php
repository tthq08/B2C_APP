    ...
    function promGoodsPrice($goods,$spec = '')
    {
        ...
        switch ($prom['p_type']){
            ...
            case '2':
                ...
                break;
            case  '3':
                $promInfo = getDiscountInfo($prom['p_id'],$goods);//优惠促销
                break;
        }
        //根据促销类型对活动商品进行计算，类型为团购和抢购时则直接调取固定金额，为优惠促销时则根据优惠类型计算优惠的价格
        //dump($promInfo);die;
        if( $prom['p_type'] == 1 || $promInfo['p_type'] == 2 ){
            if( $promInfo['goods_spec'] == '' || $spec == '' ){
                $newPrice = $promInfo['price'];
            }else{
                $newPrice = $promInfo['goods_spec'][$spec];
            }
        }elseif( $promInfo['p_type'] == 3 ){
             ...
             */
            if( $discount_type == 1 ) {
                $newPrice = number_format($newPrice*$promInfo['expression']/100,2);
            }elseif( $discount_type == 2 ){
                $newPrice = number_format($newPrice-$promInfo['expression'],2);
            }
        }
        return $newPrice;
    }
