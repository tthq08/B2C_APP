...
use app\mshop\model\Goods as GoodsModel;
...
class Goods extends HomeBase
{
	public function goodsList($id)
	{
        ...
        $this->assign($sort);
        if ($cate_id==0) {
            $filter_goods_id = Db::name('shop_goods') ->field('GROUP_CONCAT(id) as id')->where(['status'=>1,'trash'=>0])->find();
        }else{
            // 获取分类下的所有子分类ID
            $ids = Db::name('goods_category') ->where("FIND_IN_SET($cate_id,parent_id_path)")->where('is_show',1)  ->column('id','id');
            // dump($ids);
            $ids = implode(',', $ids);
            $ids = $ids.','.$ids;
            // 获取分类下所有商品的id,包括所有子分类中的商品
            $filter_goods_id = Db::name('shop_goods') ->field('GROUP_CONCAT(id) as id') ->where('cat_id','IN',$ids) ->where(['status'=>1,'trash'=>0])->find();

            
        }
        $filter_goods_id = $filter_goods_id['id'];
        ...
        // 输出已选中条件
        $filter_menu = model('goods') -> getFilterMenu($filter_param,'mshop/Goods/goodsList');
        $this->assign('filter_menu',$filter_menu);


        $filter_price = model('goods') -> getFilterPrice($filter_goods_id,$filter_param,'mshop/Goods/goodsList');
        $this->assign('filter_price',$filter_price);

        // 过滤已选中属性
        $filter_attr = model('goods') -> getFilterAttr($filter_goods_id,$filter_param,'mshop/Goods/goodsList');
        $this->assign('filter_attr',$filter_attr);

        // 输出品牌列表
        $filter_brand = model('goods') -> getFilterBrand($filter_goods_id,$filter_param,'mshop/Goods/goodsList');
	...
	public function goodsInfo($id,$pannel='')
	{
		if(empty($id)){
			$this->error('参数错误');
		}
        $this->assign('pannel',$pannel);
		$param = explode('--', $id);
		$goods_id = $param[0];
        ...
        if ($goodsInfo['is_audit']!==1) {
            $this->error('当前商品审核未通过，不可访问。');
        }
		...
		if ($spec_list) {
			...
			//默认的规格金额
            $selectedSpecPrice = getTableValue('shop_spec_price',['key_sign'=>$spec_curr,'goods_id'=>$goods_id],'price');
			...
			foreach ($spec_id as $key => $spec) {
				...
				foreach ($item as $k => $v) {
					...
					$item[$k]['href'] = url('mshop/Goods/goodsInfo',$url);
		...
		$this->assign('filter_spec',$filter_spec);
         ...
         // 检测商品当前是否在活动之中
        $is_promo = Db::name('shop_promotion') ->where('FIND_IN_SET('.$goods_id.',goods)') ->where('start_time','<= time',time()) ->where('end_time','> time',time()) ->find();
        ...
        if ($promo_goods) {
            ...
            if ($promo_spec_price) {
                ...
                if (in_array($spec_curr, $promo_specs)) {
                    $this->redirect('mshop/goods/activity',['id'=>$id]);
                }
            }
        }
        ...
        //获取分销比例
        $distribution = Db::name('user_distribution_config')->where('status',1)->value('level_proportion');
        if( !empty($distribution) ){
            ...
            $this->assign('distribution',rtrim($distribution_str,'+'));
        }

        $new_comment['img'] = is_array($new_comment['img'])?$new_comment['img'][0]:$new_comment['img'];
        $this->assign('comment',$new_comment);
        // if( $pannel == 'open' ){
        //     $this->fetch('ajax_goodsinfo');
        // }
        return $this->fetch();
	}
    ...
    public function activity($id,$pannel='')
    {
        ...
        if ($promo_goods) {
            ...
            if (!empty($promo_spec_price)) {
                $promo_specs = array_keys($promo_spec_price);
                if (!in_array($spec_curr, $promo_specs)) {
                    $this->redirect('mshop/goods/goodsInfo',['id'=>$id]);
                }
            }else{
                $promo_goods_price = $promo_goods['price'];
            }
        }else{
            $this->redirect('mshop/goods/goodsInfo',['id'=>$id]);
        }
    ...
    public function reply_comment($id=''){
        if( request()->isPost() ){
            ...
            if($is_ok){
                ...
                // $this->redirect('mshop/goods/vinfo', ['id' => $postData['note_id']]);
            }else{
                $this->error('回复失败'); 
            }
        }
