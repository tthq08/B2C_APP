...
class Wxwall extends Controller
{
	...
	{
		$this->assign('timeStamp', NOW_TIME);
		$msgs = Db::name('wex_wall_msg')->where('wall_id',tb_config('wex_wall_sign',1)) ->select();
	...
	{
		$users = Db::name('wex_wall_user') ->field('id,openid,headimgurl as avatar,nickname as name') ->where('wall_id',tb_config('wex_wall_sign',1)) ->select();
		if ($users) {
