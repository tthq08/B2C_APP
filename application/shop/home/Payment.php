...
class Payment extends HomeBase
{
    ...
    public function getCode(){        
        ...
        $serial_id = input('serial_id/d'); // 订单id
        ...
        if( $order['is_pay'] == 1 ){
        	$this->error('此订单，已完成支付!','shop/user/order_list');
        }
        //  订单支付提交
        $pay_radio = request()->param('pay_radio');
        $config_value = parse_url_param($pay_radio); // 类似于 pay_code=alipay&bank_code=CCB-DEBIT 参数
        $order['money'] = round($order['payable_price']+$order['postage']-$order['change_mny'],2);
        // 微信JS支付
    ...
    // 服务器点对点 // http://www.a.com/index.php/Home/Payment/notifyUrl        
    ...
    // 页面跳转 // http://www.a.com/index.php/Home/Payment/returnUrl        
...
//修改用户积分
    ...
    protected function order_pay_status($serial_sn,$trade_no){
        Db::name('shop_order_pay')->where('serial_sn',$serial_sn)->update(['is_pay'=>1,'pay_time'=>date('Y-m-d H:i:s', NOW_TIME),'pay_sn'=>$trade_no]);
    }
