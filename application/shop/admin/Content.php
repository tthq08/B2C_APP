...
class Content extends AdminBase
{
    ...
    public function save()
    {
        if (request()->isPost()) {
            ...
            $model_info = Db::name('shop_model') ->where('id',$data['model']) ->find();
            ...
            if ($model_info['type']==2) {       //独立模型
                ...
                $sys_field = Db::name('shop_field') ->where($sys_where) ->column('name');
                foreach ($data as $key => $val) {
                    if (is_array($val)) {
                        foreach ($val as $k => $v) {
                            if (empty($v)) {
                ...
                // dump($attach_data);
                $result = Db::name('shop_content') ->insertGetId($base_data);
                if ($result!==false) {
    ...
    public function edit()
    {
        ...
        $dbField = db('shop_field','',false);
        if ($model_info['type']==2) {
            $content = Db::name($dbContent) ->where('id',$id) ->find();
            $where['model'] = $model_info['id'];
        }else{
            $content_base = Db::name('shop_content') ->where('id',$id) ->find();
        ...
        foreach ($fields as $key => $field) {
            ...
            // dump($options);
            $form_field[] = [
                $field['title'],$field['type'],$field['name'],$content[$field['name']],'',$field['tips'],$options,$required
            ];
        }
        $JCreater = new JCreater();
        $form['form_Html'] = $JCreater->form_build($form_field);
    ...
    public function update()
    {
        if (request()->isPost()) {
            ...
            if ($model_info['type']==2) {       //独立模型
                ...
                $result = Db::name('shop_content') ->where('id',$id) ->update($base_data);
                if ($result!==false) {
    ...
    public function del()
    {
        ...
        $model_info = Db::name('shop_model') ->where('name',$model) ->find();
        $dbContent = $model_info['type'] == 2?str_replace(config('database.prefix'), '', $model_info['table']):'shop_content';
