...
use app\shop\model\Model as ContentModel;
use app\shop\model\Field as FieldModel;
use think\Db;
...
class Model extends AdminBase
{
    ...
    public function index()
    {
        ...
        */
        ...
        // 表格数据，分页，每页10条
        $models = Db::name('shop_model') ->order(['id'=>'DESC']) ->paginate(tb_config('list_rows',1,$this->lang));  //取出当前语言版本下的所有配置项
    ...
    public function save()
    {
        if (request()->isPost()) {
            ...
            $data['table'] = empty($data['table'])?config('database.prefix').'shop_'.$data['name']:$data['table'];
    ...
    public function edit($id)
    {
        ...
        $model = Db::name('shop_model') ->where('id',$id) ->find();
        $field_str = '';
    ...
    public function update()
    {
        if (request()->isPost()) {
            ...
            if ($res) {
                // 同时修改相关的菜单的状态
                $map = ['module'=>'shop','attach'=>'model='.$data['name'],'level'=>2];
    ...
    public function del($id)
    {
        ...
        if (ContentModel::deleteTable($id)) {
            // 删除主表中的文档
            if (false === Db::name('shop_content')->where('model', $id)->delete()) {
            ...
            // 删除字段数据
            if (false !== Db::name('shop_field')->where('model', $id)->delete()) {
                $res = Db::name('shop_model') ->delete($id);
    ...
    public function switchs($id)
    {
        if (request()->isPost()) {
            $data = request()->post();
            $model = Db::name('shop_model') ->where('id',$id) ->value('name');
            $res = Db::name('shop_model') ->where('id',$id) ->setField('status',$data['val']);
            if ($res!==false) {
                // 同时修改相关的菜单的状态
                $map = ['module'=>'shop','attach'=>'model='.$model,'level'=>2];
    ...
    public function field($id)
    {
        ...
        */
        ...
        // 表格数据，分页，每页10条
        $fields = Db::name('shop_field') ->where('model',$id) ->order(['sort'=>'ASC','id'=>'ASC']) ->paginate(tb_config('list_rows',1,$this->lang));  //取出当前语言版本下的所有配置项
    ...
    public function field_save()
    {
        if (request()->isPost()) {
            ...
            $model = Db::name('shop_model') ->where('id',$data['model']) ->find();
    ...
    public function field_edit($id)
    {
        ...
        $field_types = tb_config('form_item_type',1,$this->lang_id);
        $field_info = Db::name('shop_field') ->where('id',$id) ->find();
    ...
    public function field_sort($id)
    {
        if (request()->isPost()) {
            $data = request()->post();
            $res = Db::name('shop_field') ->where('id',$id) ->setField($data['field_name'],$data['field']);
            if ($res!==false) {
                sys_log(lang('comm_update_success'),1);  //操作结果写入系统日志
                $this->success(lang('comm_update_success'));
            } else {
    ...
    public function field_tbshow($id)
    {
        if (request()->isPost()) {
            $data = request()->post();
            $res = Db::name('shop_field') ->where('id',$id) ->setField('table_show',$data['val']);
            if ($res!==false) {
                sys_log(lang('comm_update_success'),1);  //操作结果写入系统日志
                $this->success(lang('comm_update_success'));
            } else {
    ...
    public function field_show($id)
    {
        if (request()->isPost()) {
            $data = request()->post();
            $res = Db::name('shop_field') ->where('id',$id) ->setField('show',$data['val']);
            if ($res!==false) {
                sys_log(lang('comm_update_success'),1);  //操作结果写入系统日志
                $this->success(lang('comm_update_success'));
            } else {
    ...
    public function field_switch($id)
    {
        if (request()->isPost()) {
            $data = request()->post();
            $res = Db::name('shop_field') ->where('id',$id) ->setField('status',$data['val']);
            if ($res!==false) {
                sys_log(lang('comm_update_success'),1);  //操作结果写入系统日志
                $this->success(lang('comm_update_success'));
            } else {
