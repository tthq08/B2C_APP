...
class Database
{
     ...
     * @param integer $export_data 是否导出数据
     * @param integer $start 起始行数
     * @param integer $start 数据总行数
     * @author 蔡伟明 <314013107@qq.com>
     * @return string
     */
    private static function getSql($table = '', $export_data = 0, $start = 0, $rows = 1000)
    {
        $sql  = "";
        if (Db::query("SHOW TABLES LIKE '%{$table}%'")) {
            ...
            if ($export_data) {
                ...
                // 备份数据记录
                $result = Db::query("SELECT * FROM `{$table}` LIMIT {$start}, {$rows}");
                foreach ($result as $row) {
                    $row = array_map('addslashes', $row);
                    $sql .= "INSERT INTO `{$table}` VALUES ('" . str_replace(array("\r","\n"),array('\r','\n'),implode("', '", $row)) . "');\n";
                }
     ...
     * @return string
     */
    public static function getSqlWhere($table = '', $where='')
    {
        $sql  = "";
        if (Db::query("SHOW TABLES LIKE '%{$table}%'")) {
			$sql .= "-- -----------------------------\n";
			...
			foreach ($result as $row) {
				...
				unset($keys[0],$values[0]);
				$fields = implode("`,`",$keys);
				$table_val = implode("','",$values);
				$sql .= "INSERT INTO `{$table}`(`{$fields}`) VALUES ('{$table_val}');\n";
				// $sql .= "INSERT INTO `{$table}`({$fields}) VALUES ('" . str_replace(array("\r","\n"),array('\r','\n'),implode("', '", $row)) . "');\n";
			}
