 ...
 * 支付 逻辑定义
 * Class AlipayPayment
 * @package plugins\payment\alipay
 */
use think\Db;
class alipay
{
    ...
    public function  __construct() {
        ...
        $paymentPlugin = getPayList(); // 找到支付插件的配置
        $config_value = $paymentPlugin['alipay']['config']; // 配置反序列化
        $this->alipay_config['alipay_pay_method']= $config_value[4]['value']; // 1 使用担保交易接口  2 使用即时到帐交易接口s
        $this->alipay_config['partner']       = $config_value[2]['value'];//合作身份者id，以2088开头的16位纯数字
        $this->alipay_config['seller_email']  = $config_value[0]['value'];//收款支付宝账号，一般情况下收款账号就是签约账号
        $this->alipay_config['key']	      = $config_value[1]['value'];//安全检验码，以数字和字母组成的32位字符
    ...
    function get_code($order, $config_value)
    {
        ...
        $parameter = array(
            ...
            "notify_url"	=> "http://".$_SERVER['HTTP_HOST'].url('shop/Payment/notifyUrl',array('pay_code'=>'alipay')) , //服务器异步通知页面路径 //必填，不能修改
            "return_url"	=> "http://".$_SERVER['HTTP_HOST'].url('shop/User/order_list'),  //页面跳转同步通知页面路径
            "out_trade_no"	=> $order['serial_sn'], //商户订单号
            "subject"	=> tb_config('pay_cust_title',1,1), //订单名称 可以中文
    ...
    function response()
    {
        ...
        if($verify_result) //验证成功
        {
            ...
            {
                if( strpos($order_sn,'cz') !== false ){
                    // 充值订单回调
                    $back = api('shop','Recharge','CZNotifyCallback',[$order_sn,$trade_no,$_POST]);
                }else{
                    $back = update_pay_status($order_sn,$trade_no); // 修改订单支付状态
                }
                if( $back === true ){
                    return "success";
                }else{
