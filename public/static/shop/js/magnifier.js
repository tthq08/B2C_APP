(function($){
	...
	window[fnName] = function(magnifierAttr){
		...
		_this.eqImg = function(){
			...
			var img = new Image(),
				src = _this.thumbnail.find("img").eq(magnifier.index).attr('max-img');
		...
		_this.imgMove = function(_boole){
			...
			var _deviation = Math.ceil(magnifier.width / thumbnailImg.width() /1.5);
		...
		_this.assembly.find(">*").on('click',function(){
			...
			if(magnifier.index == 0) { magnifier.index = 1; }
			// console.log(magnifier.index)
