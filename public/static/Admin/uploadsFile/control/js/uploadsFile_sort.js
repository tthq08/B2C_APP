		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="/static/uploadsFile/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.createCorePlug = function(){
				var params = {
						...
						var funBindDelEvent = function(){
							if($(".file_del").length>0){
								...
								$(".file_del").click(function() {
									var f_index = $(this).attr("data-index");
									$('#'+para.pic_obj+'_'+f_index).remove();
									ZYFILE.funDeleteFile(parseInt(f_index), true);
									return false;	
								});
							}
					...
					onSuccess: function(file, response) {
						...
						$("#photo-container").append("<li id='"+para.pic_obj+"_"+file.index+"'><img src='"+$.parseJSON(response).url+"' /><input type='text' name='sort["+file.index+"]' value='' class='layui-input' placeholder='排序'><input type='hidden' name='"+para.pic_obj+"["+file.index+"]' value='"+$.parseJSON(response).url+"' ><div class='file_bar '><divstyle='padding:5px;'><p class='file_name'>是否删除？</p><span class='file_del' data-index='0' title='删除'></span>   </div>  </div></li>");
						// $("#uploadInf").append("<p>上传成功，文件地址是：" + response + "</p>"); //关闭时间2015-01-20 augustdai
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
